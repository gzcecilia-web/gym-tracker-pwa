...
export function ExerciseAccordion({
  ...
  return (
    <section className={`rounded-r-lg border bg-surface p-4 shadow-card transition-all duration-200 ease-out ${complete ? 'border-accent/25' : 'border-line'}`}>
      ...
      {open ? <div className="mt-4 border-t border-[#F1EFEB] pt-4">{children}</div> : null}
    </section>
  );
}
  ...
  return (
    <div className="rounded-r-md border border-line border-l-4 border-l-accent bg-accent/8 p-4">
      ...
      <div className="space-y-2">
        {[1, 2, 3].map((drop) => (
          <div key={drop} className="grid grid-cols-[56px,1fr] items-center gap-2 rounded-r-sm bg-surface/80 px-2 py-1">
